...
1. 编译期固定：key 写在固件源码或编译参数中。实现简单，但泄露风险高，不利于量产。
2. 运行期配置：上层在启动时调用 `ping_pong_set_security()` 传入 key。灵活，适合由业务层、配置区或安全芯片提供 key。
3. 出厂写入：每台设备出厂时写入唯一 key 或一组 key 到 Flash / EEPROM / OTP / 安全芯片。适合正式产品。
4. 端口层回调：PingPong 核心不直接保存 key，只在需要计算 MAC 时调用端口层接口。适合 key 不允许暴露给协议栈的场景。

当前建议：先采用“运行期配置”作为 API 形式；具体 key 来源由上层决定。这样既不把 key 写死在库里，也方便后续接入出厂写入或安全芯片。
...
- 每次或定期把 counter 保存到非易失存储
- 设备重启后继续使用更大的 counter
- 可以防止重启后的旧包重放
- 成本是实现复杂，需要考虑 Flash 擦写寿命、掉电时机和异常恢复

折中方案：

- counter 按块分配，例如每次启动从 Flash 读取一个基值，然后立即把 Flash 中的基值增加 1024 或 4096
- RAM 中在这个区间内递增
- 这样不需要每发一个包都写 Flash，同时能降低重放风险

当前建议：正式生产应支持掉电保存或块分配策略；如果第一版实现需要简化，可以先支持运行期传入 `tx_counter` 初始值，由上层负责持久化策略。
...
1. key 最终来源：运行期配置、出厂写入、配置区读取，还是安全芯片。
2. counter 持久化策略：上层持久化、核心辅助块分配，还是第一版暂不掉电保存。
