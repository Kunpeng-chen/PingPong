...
1. 每个阶段必须使用干净实现分支推进。
2. 实现分支必须从最新 `main` 创建。
3. 不得直接从 `docs/production-readiness-plan` 或其他文档分支开发功能。
4. 一个 PR 只实现一个 Phase 或一个明确的独立任务。
5. PR 中只包含当前任务必要的代码、测试和少量说明。
6. 不得混入无关重构、历史临时提交或多个 Phase 的功能。
7. CI 未通过不得合并。
8. PR 不可合并时不得强行推进，应重新从 `main` 创建干净分支。
9. 合并到 `main` 后必须检查 README。
10. 计划状态不得提前标记完成。
