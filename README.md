...
面向嵌入式场景的轻量级 Ping-Pong 协议核心模块。它只负责协议状态机、收发时序、超时与重试、基础统计、设备身份过滤和通知回调；时钟、无线收发、日志和业务决策都交给上层端口适配。
...
- 内建超时、重试、冲突检测和 Master 自动连续运行
- v2 包格式包含 `network_id`、`src_id`、`dst_id`，用于网络隔离和设备地址过滤
...
3. 按需调用 `ping_pong_set_config()` 覆盖默认超时、重试、自动重启和 identity 参数。
